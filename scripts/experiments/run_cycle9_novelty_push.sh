...
run_routed_condition() {
  ...
  python scripts/experiments/run_routed_frontier_external.py \
    ...
    --saebench-results-path "$HUSAI_TMP_ROOT/husai_saebench_probe_results_cycle9_routed" \
    --saebench-model-cache-path "$SAEBENCH_MODEL_CACHE_PATH" \
    --cebench-artifacts-path "$HUSAI_TMP_ROOT/ce_bench_artifacts_cycle9_routed" \
...
run_assignment_condition() {
  ...
  python scripts/experiments/run_assignment_consistency_v3.py \
    ...
    --cebench-matched-baseline-summary "$default_baseline" \
    --saebench-datasets "$SAEBENCH_DATASETS" \
    --saebench-results-path "$HUSAI_TMP_ROOT/husai_saebench_probe_results_cycle9_assignment" \
    --saebench-model-cache-path "$SAEBENCH_MODEL_CACHE_PATH" \
    --cebench-artifacts-path "$HUSAI_TMP_ROOT/ce_bench_artifacts_cycle9_assignment" \
...
python scripts/experiments/run_ood_stress_eval.py \
  ...
  --device cuda \
  --dtype float32 \
  --results-path "$HUSAI_TMP_ROOT/husai_saebench_probe_results_ood_cycle9" \
