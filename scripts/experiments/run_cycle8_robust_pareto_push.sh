...
RUN_DIR="results/experiments/cycle8_robust_pareto_push/${RUN_ID}"
mkdir -p "$RUN_DIR"
LOG_PATH="$RUN_DIR/cycle8.log"
exec > >(tee -a "$LOG_PATH") 2>&1

echo "[cycle8] run_id=$RUN_ID"
echo "[cycle8] assignment_update_interval=$ASSIGN_UPDATE_INTERVAL"
echo "[cycle8] waiting for conflicting experiment runners to finish"
while pgrep -f "run_cycle7_pareto_push.sh|run_cycle6_saeaware_push.sh|run_cycle5_external_push.sh|run_cycle4_followups_after_queue.sh|run_architecture_frontier_external.py|run_routed_frontier_external.py|run_assignment_consistency_v3.py|run_external_metric_scaling_study.py" >/dev/null 2>&1; do
  date -u +"[cycle8] %Y-%m-%dT%H:%M:%SZ another runner active"
  sleep "$WAIT_SECONDS"
done

echo "[cycle8] git pull"
git pull origin main

echo "[cycle8] stage1: robust routed Pareto sweep"
ROUTED_SWEEP_ROOT="results/experiments/phase4b_routed_frontier_external_sweep_cycle8_robust"
...
run_routed_condition() {
  ...
  echo "[cycle8][routed] condition=${tag} dsae=${dsae} k=${k} experts=${experts} lr=${lr} epochs=${epochs} noise=${noise_std} cons=${consistency_coef} div=${diversity_coef}"
  ...
  python scripts/experiments/run_routed_frontier_external.py \
    ...
    --saebench-results-path /tmp/husai_saebench_probe_results_cycle8_routed \
    --saebench-model-cache-path "$SAEBENCH_MODEL_CACHE_PATH" \
    --cebench-artifacts-path /tmp/ce_bench_artifacts_cycle8_routed \
  ...
  if [[ -n "$run_dir" ]]; then
    ROUTED_RUNS+=("$run_dir")
    echo "[cycle8][routed] done ${tag} -> $run_dir"
  fi
}

run_routed_condition "b0" 1536 40 6 0.02 0.02 0.0008 12 0.0 0.0 0.0 512
run_routed_condition "r1" 1536 40 6 0.02 0.02 0.0008 12 0.02 0.10 0.0 512
run_routed_condition "r2" 1536 40 6 0.02 0.02 0.0008 12 0.02 0.10 0.02 512
run_routed_condition "r3" 2048 40 6 0.02 0.02 0.0007 12 0.015 0.08 0.02 768
run_routed_condition "r4" 1024 48 4 0.02 0.02 0.0008 12 0.03 0.12 0.015 512

echo "[cycle8] stage2: assignment-v3 external-aware Pareto sweep"
ASSIGN_SWEEP_ROOT="results/experiments/phase4d_assignment_consistency_v3_cycle8_robust"
...
run_assignment_condition() {
  ...
  echo "[cycle8][assignment] condition=${tag} dsae=${dsae} k=${k} epochs=${epochs} lr=${lr}"
  KMP_DUPLICATE_LIB_OK=TRUE MPLCONFIGDIR=/tmp/mpl \
  python scripts/experiments/run_assignment_consistency_v3.py \
    ...
    --cebench-matched-baseline-summary "$default_baseline" \
    --saebench-datasets "$SAEBENCH_DATASETS" \
    --saebench-results-path /tmp/husai_saebench_probe_results_cycle8_assignment \
    --saebench-model-cache-path "$SAEBENCH_MODEL_CACHE_PATH" \
    --cebench-artifacts-path /tmp/ce_bench_artifacts_cycle8_assignment \
    ...
    --external-candidate-weight-saebench 0.75 \
    --external-candidate-weight-cebench 0.15 \
    --external-candidate-weight-alignment 0.05 \
    --external-candidate-weight-ev 0.05 \
    --weight-internal-lcb 0.20 \
    --weight-ev 0.05 \
    --weight-saebench 0.55 \
    --weight-cebench 0.20 \
    --force-rerun-external \
    --require-external \
    --min-saebench-delta -0.02 \
    --min-cebench-delta -35.5 \
  ...
  if [[ -n "$run_dir" ]]; then
    ASSIGN_RUNS+=("$run_dir")
    echo "[cycle8][assignment] done ${tag} -> $run_dir"
  fi
}

run_assignment_condition "a1" 2048 40 24 0.0006
run_assignment_condition "a2" 2048 48 24 0.0005
run_assignment_condition "a3" 3072 48 24 0.0004

echo "[cycle8] stage3: grouped LCB reselection"
...
if [[ -z "$FRONTIER_BASE" || ! -f "$FRONTIER_BASE/results.json" ]]; then
  echo "[cycle8][error] missing frontier base results (phase4b_architecture_frontier_external_multiseed)"
  exit 2
fi
if [[ -z "$SCALING_RUN" || ! -f "$SCALING_RUN/results.json" ]]; then
  echo "[cycle8][error] missing scaling results (phase4e_external_scaling_study_multiseed)"
  exit 2
fi
...
SELECTOR_ARGS=(
  ...
  --weight-saebench 0.75
  --weight-cebench 0.20
  --weight-train-ev 0.05
  --output-dir results/experiments/release_candidate_selection_cycle8_robust
)
...
SELECTOR_RUN="$(ls -1dt results/experiments/release_candidate_selection_cycle8_robust/run_* 2>/dev/null | head -n1 || true)"
SELECTED_JSON="$SELECTOR_RUN/selected_candidate.json"
if [[ -z "$SELECTOR_RUN" || ! -f "$SELECTED_JSON" ]]; then
  echo "[cycle8][error] selector output missing selected_candidate.json"
  exit 2
fi
...
SELECTOR_ARGS_MIN3=(
  ...
  --weight-cebench 0.20
  --weight-train-ev 0.05
  --output-dir results/experiments/release_candidate_selection_cycle8_robust_min3
)
for rr in "${ROUTED_RUNS[@]}"; do
...
if [[ -z "$BEST_CHECKPOINT" || ! -f "$BEST_CHECKPOINT" ]]; then
  echo "[cycle8][error] selected checkpoint missing: $BEST_CHECKPOINT"
  exit 2
fi

echo "[cycle8] stage4: OOD + strict release gate"
KMP_DUPLICATE_LIB_OK=TRUE MPLCONFIGDIR=/tmp/mpl \
python scripts/experiments/run_ood_stress_eval.py \
  --checkpoint "$BEST_CHECKPOINT" \
  --architecture "$BEST_ARCH" \
  --sae-release husai_cycle8_ood \
  --model-name pythia-70m-deduped \
  --hook-layer "$BEST_HOOK_LAYER" \
  --hook-name "$BEST_HOOK_NAME" \
  --device cuda \
  --dtype float32 \
  --results-path /tmp/husai_saebench_probe_results_ood_cycle8 \
...
echo "[cycle8] complete"
echo "[cycle8] manifest=$RUN_DIR/manifest.json"
