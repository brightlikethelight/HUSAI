#!/usr/bin/env python3
"""Run a matched-budget routed TopK frontier on external benchmarks.

This introduces a RouteSAE-inspired architecture family under matched budget by
training a TopK SAE with an auxiliary router that gates features by expert group.

The output format mirrors existing frontier scripts so selector/gate tooling can
consume this run without special handling.
"""
...
def build_expert_slices(d_sae: int, num_experts: int) -> list[tuple[int, int]]:
    ...
    for expert in range(num_experts):
        width = base + (1 if expert < rem else 0)
        end = start + width
        out.append((start, end))
        start = end

    if out[-1][1] != d_sae:
        raise RuntimeError(f"Expert slices did not cover d_sae={d_sae}")
    return out
...
def build_router_mask(expert_ids: torch.Tensor, d_sae: int, slices: list[tuple[int, int]]) -> torch.Tensor:
    ...
    for expert, (s, e) in enumerate(slices):
...
def train_routed_topk(
    *,
    ...
    model = HUSAITopKSAE(d_model=d_model, d_sae=d_sae, k=min(k, d_sae)).to(device=torch.device(device), dtype=dtype)
    router = nn.Linear(d_model, num_experts).to(device=torch.device(device), dtype=dtype)
    ...
    optimizer = torch.optim.Adam([*model.parameters(), *router.parameters()], lr=learning_rate)
    expert_slices = build_expert_slices(d_sae=d_sae, num_experts=num_experts)
    ...
    for _ in range(epochs):
        ...
        for (batch_cpu,) in loader:
            ...
            target = torch.full_like(usage, 1.0 / float(num_experts))
    ...
    with torch.no_grad():
        ...
        l0 = float((feats_routed != 0).float().sum(dim=-1).mean().item())

        usage = probs.mean(dim=0)
        target = torch.full_like(usage, 1.0 / float(num_experts))
        balance_l2 = float(torch.linalg.vector_norm(usage - target).item())
        entropy = float((-(probs * torch.log(probs.clamp_min(1e-9))).sum(dim=-1).mean()).item())
    ...
    diagnostics = RouteDiagnostics(
        ...
        group_size_mean=float(np.mean([e - s for s, e in expert_slices])),
    )

    return model.cpu(), router.cpu(), metrics, diagnostics
...
def write_checkpoint(
    ...
    *,
    ...
    payload = {
        ...
        "consistency_objective": "route_expert_gating",
        "routing": {
            "num_experts": num_experts,
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Matched-budget routed TopK frontier external run")
    ...
    parser.add_argument(
        ...
        type=Path,
        default=PROJECT_ROOT / "results" / "cache" / "external_benchmarks" / "husai_saebench_probe_results_frontier_routed",
    )
    parser.add_argument("--saebench-model-cache-path", type=Path, default=DEFAULT_SAEBENCH_MODEL_CACHE)
    parser.add_argument(
        "--cebench-artifacts-path",
        type=Path,
        default=PROJECT_ROOT / "results" / "cache" / "external_benchmarks" / "ce_bench_artifacts_frontier_routed",
    )

    parser.add_argument(
        "--output-dir",
        type=Path,
        default=PROJECT_ROOT / "results" / "experiments" / "phase4b_routed_frontier_external",
    )
    ...
    for seed in seeds:
        condition_id = f"routed_topk_seed{seed}"
        ckpt_dir = checkpoints_dir / condition_id
        ckpt_path = ckpt_dir / "sae_final.pt"
        ckpt_dir.mkdir(parents=True, exist_ok=True)

        model, router, train_metrics, route_diag = train_routed_topk(
        ...
        rec: dict[str, Any] = {
            ...
            "route_diagnostics": asdict(route_diag),
            "saebench": None,
            "cebench": None,
        }
        ...
        if args.run_saebench:
            ...
            command = [
                ...
                "routed_topk",
                "--sae-release",
                f"husai_{run_id}_{condition_id}",
                "--model-name",
                args.model_name,
                "--hook-layer",
                str(args.hook_layer),
    ...
    print("Routed frontier run complete")
