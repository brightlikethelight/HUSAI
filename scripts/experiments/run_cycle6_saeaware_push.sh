...
echo "[cycle6] git pull"
git pull origin main

echo "[cycle6] stage1: routed expert_topk expansion sweep"
ROUTED_SWEEP_ROOT="results/experiments/phase4b_routed_frontier_external_sweep_cycle6"
...
run_routed_condition() {
  ...
  echo "[cycle6][routed] condition=${tag} dsae=${dsae} k=${k} experts=${experts} bal=${bal} ent=${ent} lr=${lr} epochs=${epochs}"
  ...
  python scripts/experiments/run_routed_frontier_external.py \
    ...
    --saebench-results-path "$HUSAI_TMP_ROOT/husai_saebench_probe_results_cycle6_routed" \
    --saebench-model-cache-path "$SAEBENCH_MODEL_CACHE_PATH" \
    --cebench-artifacts-path "$HUSAI_TMP_ROOT/ce_bench_artifacts_cycle6_routed" \
  ...
  if [[ -n "$run_dir" ]]; then
    ROUTED_RUNS+=("$run_dir")
    echo "[cycle6][routed] done ${tag} -> $run_dir"
  fi
}

run_routed_condition "r1" 2048 32 8 0.02 0.02 0.0010 10
run_routed_condition "r2" 2048 32 12 0.02 0.02 0.0010 10
run_routed_condition "r3" 2048 48 8 0.02 0.02 0.0007 12
run_routed_condition "r4" 3072 48 12 0.02 0.02 0.0005 14

echo "[cycle6] stage2: assignment-v3 SAEBench-aware external checkpoint policy sweep"
ASSIGN_SWEEP_ROOT="results/experiments/phase4d_assignment_consistency_v3_cycle6_saeaware"
...
run_assignment_condition() {
  ...
  echo "[cycle6][assignment] condition=${tag} dsae=${dsae} k=${k} epochs=${epochs} lr=${lr}"
  KMP_DUPLICATE_LIB_OK=TRUE MPLCONFIGDIR="$HUSAI_MPLCONFIGDIR" \
  python scripts/experiments/run_assignment_consistency_v3.py \
    ...
    --cebench-matched-baseline-summary "$default_baseline" \
    --saebench-datasets "$SAEBENCH_DATASETS" \
    --saebench-results-path "$HUSAI_TMP_ROOT/husai_saebench_probe_results_cycle6_assignment" \
    --saebench-model-cache-path "$SAEBENCH_MODEL_CACHE_PATH" \
    --cebench-artifacts-path "$HUSAI_TMP_ROOT/ce_bench_artifacts_cycle6_assignment" \
    --external-checkpoint-policy external_score \
    --external-checkpoint-candidates-per-lambda 4 \
    --external-candidate-require-both \
    --external-candidate-min-saebench-delta -0.02 \
    --external-candidate-min-cebench-delta -20.0 \
    --external-candidate-weight-saebench 0.60 \
    --external-candidate-weight-cebench 0.30 \
    --external-candidate-weight-alignment 0.05 \
    --external-candidate-weight-ev 0.05 \
    --weight-internal-lcb 0.35 \
    --weight-ev 0.10 \
    --weight-saebench 0.35 \
    --weight-cebench 0.20 \
    --force-rerun-external \
    --require-external \
    --min-saebench-delta -0.02 \
    --min-cebench-delta -20.0 \
  ...
  if [[ -n "$run_dir" ]]; then
    ASSIGN_RUNS+=("$run_dir")
    echo "[cycle6][assignment] done ${tag} -> $run_dir"
  fi
}

run_assignment_condition "a1" 2048 32 18 0.0007
run_assignment_condition "a2" 3072 48 22 0.0005

echo "[cycle6] stage3: grouped LCB reselection"
...
if [[ -z "$FRONTIER_BASE" || ! -f "$FRONTIER_BASE/results.json" ]]; then
  echo "[cycle6][error] missing frontier base results (phase4b_architecture_frontier_external_multiseed)"
  exit 2
fi
if [[ -z "$SCALING_RUN" || ! -f "$SCALING_RUN/results.json" ]]; then
  echo "[cycle6][error] missing scaling results (phase4e_external_scaling_study_multiseed)"
  exit 2
fi
...
SELECTOR_ARGS=(
  ...
  --weight-saebench 0.65
  --weight-cebench 0.30
  --weight-train-ev 0.05
  --output-dir results/experiments/release_candidate_selection_cycle6
)
...
SELECTOR_RUN="$(ls -1dt results/experiments/release_candidate_selection_cycle6/run_* 2>/dev/null | head -n1 || true)"
SELECTED_JSON="$SELECTOR_RUN/selected_candidate.json"
if [[ -z "$SELECTOR_RUN" || ! -f "$SELECTED_JSON" ]]; then
  echo "[cycle6][error] selector output missing selected_candidate.json"
  exit 2
fi
...
SELECTOR_ARGS_MIN2=(
  ...
  --group-by-condition
  --uncertainty-mode lcb
  --min-seeds-per-group 2
  --weight-saebench 0.65
  --weight-cebench 0.30
  --weight-train-ev 0.05
  --output-dir results/experiments/release_candidate_selection_cycle6_min2
)
for rr in "${ROUTED_RUNS[@]}"; do
  [[ -f "$rr/results.json" ]] && SELECTOR_ARGS_MIN2+=(--frontier-results "$rr/results.json")
done
for ar in "${ASSIGN_RUNS[@]}"; do
  [[ -f "$ar/results.json" ]] && SELECTOR_ARGS_MIN2+=(--assignment-results "$ar/results.json")
done
python scripts/experiments/select_release_candidate.py "${SELECTOR_ARGS_MIN2[@]}"
...
if [[ -z "$BEST_CHECKPOINT" || ! -f "$BEST_CHECKPOINT" ]]; then
  echo "[cycle6][error] selected checkpoint missing: $BEST_CHECKPOINT"
  exit 2
fi

echo "[cycle6] stage4: OOD + strict release gate"
KMP_DUPLICATE_LIB_OK=TRUE MPLCONFIGDIR="$HUSAI_MPLCONFIGDIR" \
python scripts/experiments/run_ood_stress_eval.py \
  --checkpoint "$BEST_CHECKPOINT" \
  --architecture "$BEST_ARCH" \
  --sae-release husai_cycle6_ood \
  --model-name pythia-70m-deduped \
  --hook-layer "$BEST_HOOK_LAYER" \
  --hook-name "$BEST_HOOK_NAME" \
  --device cuda \
  --dtype float32 \
  --results-path "$HUSAI_TMP_ROOT/husai_saebench_probe_results_ood_cycle6" \
...
echo "[cycle6] complete"
echo "[cycle6] manifest=$RUN_DIR/manifest.json"
